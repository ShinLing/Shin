...
// 資料型別(原生):數值(number)、字串(string)、布林(boolean)、未定義(undefind)、空值(null)
// 物件類別(object)、符號(symbol)
...
// 0.300000000000000004 ; 使用 IEEE 754 標準 雙精準算法(64位元)
var b = 0.1 + 0.2;
document.write("<br>" + b);
...
// 布林(boolean)只有兩種數值,true(1)與false(2)
// false、0、空字串("")or('')、NaN、null、未定義(undefined);布林值都為false(0)
// 其他值為 true(1)
...
var X;
y = 1;  // 設定名稱,給予值,javascript 會將其轉變為變數
document.write("<br" + y);
...
function cal(){
    ...
    function cal(){
        // 連續宣告變數
        // a = 5, B;宣告變數給予值時,可以不用 var 定義 ;宣告變數不予值時, 需要用var 定義1才會成立
        a =5,b =1;// 全域變數
    ...
    // let宣告的作用域 會因 {區塊敘述}而分別定義為 全域或區域 的型態
